...
# per-stock prices (from the cached scored panel -- has close prices for all 58)
p = pd.read_csv("/tmp/v5_expanding.csv", parse_dates=["date"])
px = p.pivot_table(index="date", columns="symbol", values="close").ffill()
bd = px.index[(px.index.year >= BT_START) & (px.index.year <= BT_END)]
px = px.loc[bd]

# per-stock cumulative return, each stock starting at 0% on day 1
...
# label the best and worst performing stocks at the end, for context
...
# strategy + benchmark, highlighted
...
# also save a data file: each stock's own CAGR/return over the window, for reference
