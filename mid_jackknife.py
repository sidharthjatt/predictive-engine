...
def panel():
    ...
    if _P is None:
        ...
        _P = pd.read_csv(config.require_cache(REGISTRY["midcap150"].score_cache,
...
def edge(drop=()):
    ...
    # RESEARCH-ONLY, DECLARED. This tool passes no vol20, so it could not apply a
    # participation cap even if one were selected; research_only() makes that a
    # statement rather than an accident, and STOPS the run if --profile ever
    # reaches here. See profiles.research_only and backtest_exposure's refusal.
