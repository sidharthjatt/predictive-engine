...
def main(u):
    ...
    # equally if the marker is deleted while a literal REGISTRY["mid"] subscript
    ...
    #
    # A FROZEN UNIVERSE IS NOT REACHED FROM HERE. mid is live; the 58 and 74 go
    # through frozen/make_daily_audit.py, which is deliberately left on v2 only.
    for _a in arm_reg.selected():
        audit_step.run(REGISTRY["mid"], _a)
