"""
engine_v2_final.py -- FINAL v2 STRATEGY (breadth-scaled)
=========================================================
The final, complete strategy. Everything that was validated, in one place:
...
What breadth scaling does:
  ...
  It does NOT block individual stocks (that was the mistake with the absolute
  gate) -- it only scales total exposure. There is no tunable threshold.
...
Run: python3 results/engine_v2_final.py
"""
...
def main():
    ...
    # FROZEN 58: value_at_open=False pins the pre-2026-09-04 close-valued sizing,
    # so this retired universe's published numbers cannot move. The live universes
    # take the corrected default. Same pattern as build_scores.py pinning
    # purge_mode="calendar" here. See test_exposure.backtest_exposure.
    ...
    bt.to_csv(M / "daily_trades_v1_58.csv", index=False)
    print(f"   v1 baseline trade log: {len(bt)} trades, TC Rs {bt['tc'].sum():,.0f} "
          f"-> daily_trades_v1_58.csv")
  ...
  Versus equal-weight buy & hold over the same period:
    Sharpe   {mfin['Sharpe']:>6.2f} vs {mbh['Sharpe']:>6.2f}   ({d_sh:+.2f})
