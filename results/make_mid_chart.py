...
# ------------------------------------------------------------------- chart
sub = (f"NOT A VALID RESULT -- PANEL DENSITY BLOCKER: only {n_panel} of {n_all} names are "
       f"scored, median {int(per_day.median())} priced per day. idio_vol_60/beta_60 use "
       f"rolling(60) on the UNION date index,\nso midcap non-trading days void 60 windows "
       f"each; 32.9% of rows survive. Unpriced names forward-fill flat, so breadth reads "
       f"0.202 vs 0.550 on the 58 and deployment collapses to ~20%.\n"
       ...
       # LIQUIDITY, ON THE SAME PAGE AS THE CAGR IT QUALIFIES.
       #   Measured in liquidity_participation.py and depth_compare.py. It belongs
       #   in the subtitle because the headline 29.18% is not reachable at the size
       #   these fills imply: one AIIL order is sixteen days of that stock's median
       #   volume, at the backtest's own Rs 10,00,000 -- not at some hypothetical
       #   larger fund. A reader who sees the return without this sees half of it.
       f"LIQUIDITY: at Rs 10,00,000 starting capital, 22 of 985 fills exceed 10% of "
       f"prior-20-day median volume, the largest being 1,614% on AIIL.\n"
       f"Modelling realistic depth (10% of median daily volume per level, three "
       f"levels) costs 1.80 CAGR points: 29.16% -> 27.36%, Sharpe 2.00 -> 1.90.\n"
